...
class critic:
  ...
  def build(self):
    ...
    model.compile(loss='mse',
                  optimizer=Adam(learning_rate=self.params['critic_lr']))
    return model
  ...
  def train_model_free_monte_carlo(self, states, actions, returns):
    ...
    for e in range(self.params['critic_num_epochs']):
      state_q = self.network.predict(numpy.array(states), verbose=0)
  ...
  def train_model_free_TD(self, memory, actor, meta_params, alg_params, e):
    ...
    for _ in range(alg_params['critic_num_epochs']):
      ...
      Pi = actor.network.predict(numpy.array(next_states_li), verbose=0)
      Q = self.target_network.predict(numpy.array(next_states_li), verbose=0)
      ...
      state_pred = self.network.predict(numpy.array(states), verbose=0)
