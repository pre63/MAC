...
class actor:
  ...
  def select_action(self, state):
    ...
    '''
    pr = self.network.predict(numpy.array(state).reshape(1, self.params['state_|dimension|']), verbose=0 )[0]
  ...
  def train(self, states, critic):
    ...
    '''
    state_q = critic.network.predict(numpy.array(states), verbose=0)
